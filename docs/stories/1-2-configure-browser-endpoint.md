...
- [x] [Review][Defer] `isLoopbackHost` incomplete loopback detection — `127.x.x.x` block and IPv6 variants (`::ffff:127.0.0.1`, long-form `0:0:0:0:0:0:0:1`) are shown as `[redacted-host]`. [`src/config/endpoint-config.ts`] — deferred, expanded display-only loopback classification is post-MVP
